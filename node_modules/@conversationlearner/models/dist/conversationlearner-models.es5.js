...
function serialize(value, entityValuesMap, fallbackToOriginal) {
    if (fallbackToOriginal === void 0) { fallbackToOriginal = false; }
...
function serializeNode(node, entityValues, fallbackToOriginal) {
    if (node.kind === 'text') {
        return node.leaves.map(function (n) { return n.text; }).join('');
    }
    var serializedChildNodes = node.nodes.map(function (n) { return serializeNode(n, entityValues, fallbackToOriginal); });
    if (node.kind === 'inline' && node.type === NodeTypes.Mention) {
        // This check is required becuase when input is Slate Value node is Immutable.Map object
        // but it could also be a node from value.toJSON()
        var data = typeof node.data.toJS === 'function' ? node.data.toJS() : node.data;
...
var ActionArgument = /** @class */ (function () {
    ...
    ActionArgument.prototype.renderValue = function (entityValues) {
...
var TextAction = /** @class */ (function (_super) {
    ...
    TextAction.prototype.renderValue = function (entityValues) {
        return EntityIdSerializer.serialize(this.value, entityValues);
    };
    return TextAction;
}(ActionBase));
var ApiAction = /** @class */ (function (_super) {
    ...
    ApiAction.prototype.renderArguments = function (entityValues) {
...
var CardAction = /** @class */ (function (_super) {
    ...
    CardAction.prototype.renderArguments = function (entityValues) {
        return this.arguments.map(function (aa) {
            var value = null;
            try {
                value = EntityIdSerializer.serialize(aa.value, entityValues);
...
var filledEntityValueAsString = function (fe) {
    // Print out list in friendly manner
    var group = '';
    for (var key in fe.values) {
        var index = +key;
        var prefix = '';
...
var getEntityDisplayValueMap = function (filledEntityMap) {
    return Object.keys(filledEntityMap.map).reduce(function (m, key) {
        var entityDisplayValue = filledEntityMap.EntityValueAsString(key);
...
var FilledEntityMap = /** @class */ (function () {
    ...
    FilledEntityMap.prototype.EntityValueAsList = function (entityName) {
    ...
    FilledEntityMap.prototype.EntityValueAsString = function (entityName) {
        if (!this.map[entityName]) {
            return null;
        }
        // Print out list in friendly manner
...
var ReplayErrorMissingAction = /** @class */ (function (_super) {
    ...
    function ReplayErrorMissingAction(lastUserInput) {
        var _this = _super.call(this, ReplayErrorType.MissingAction) || this;
...
var ReplayErrorMissingEntity = /** @class */ (function (_super) {
    ...
    function ReplayErrorMissingEntity(value) {
        var _this = _super.call(this, ReplayErrorType.MissingEntity) || this;
        _this.value = value;
        return _this;
    }
...
export { EntityIdSerializer, ActionTypes, ActionBase, ActionArgument, TextAction, ApiAction, CardAction, TrainingStatusCode, EntityType, makeNegative, filledEntityValueAsString, getEntityDisplayValueMap, FilledEntityMap, KeyGen, ModelUtils, ReplayErrorType, ReplayError, ReplayErrorMissingAction, ReplayErrorMissingEntity, ReplayErrorActionUnavailable, ReplayErrorEntityDiscrepancy, DialogType, DialogMode, SenderType, CL_USER_NAME_ID, MEMORY_KEY_HEADER_NAME, ScoreReason };
