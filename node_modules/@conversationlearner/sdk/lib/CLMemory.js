    ...
    SetAsync(datakey, value) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                let cacheData = this.memCache[key];
                if (cacheData == value) {
                    CLDebug_1.CLDebug.Log(`-> ${key} : ${value}`, CLDebug_1.DebugType.MemVerbose);
                }
                else {
                    // Write to memory storage (use * for etag)
                    yield CLMemory.memoryStorage.write({ [key]: { value: value, eTag: '*' } });
                    this.memCache[key] = value;
                    CLDebug_1.CLDebug.Log(`W> ${key} : ${value}`, CLDebug_1.DebugType.Memory);
                }
            }
            catch (err) {
                CLDebug_1.CLDebug.Error(err);
    ...
    DeleteAsync(datakey) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                let cacheData = this.memCache[key];
                if (!cacheData) {
                    CLDebug_1.CLDebug.Log(`-> ${key} : -----`, CLDebug_1.DebugType.MemVerbose);
                }
                else {
