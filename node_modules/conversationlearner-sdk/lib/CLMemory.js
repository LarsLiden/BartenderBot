...
class CLMemory {
    ...
    GetAsync(datakey) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (cacheData) {
                CLDebug_1.CLDebug.Log(`-< ${key} : ${cacheData}`, 'memverbose');
                return cacheData;
            }
            else {
                try {
                    ...
                    CLDebug_1.CLDebug.Log(`R< ${key} : ${this.memCache[key]}`, 'memory');
    ...
    SetAsync(datakey, value) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                let cacheData = this.memCache[key];
                if (cacheData == value) {
                    CLDebug_1.CLDebug.Log(`-> ${key} : ${value}`, 'memverbose');
                }
                else {
                    // Write to memory storage (use * for etag)
                    yield CLMemory.memoryStorage.write({ [key]: { value: value, eTag: '*' } });
                    this.memCache[key] = value;
                    CLDebug_1.CLDebug.Log(`W> ${key} : ${value}`, 'memory');
                }
            }
            catch (err) {
                CLDebug_1.CLDebug.Error(err);
    ...
    DeleteAsync(datakey) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                let cacheData = this.memCache[key];
                if (!cacheData) {
                    CLDebug_1.CLDebug.Log(`-> ${key} : -----`, 'memverbose');
                }
                else {
                    ...
                    if (!CLMemory.memoryStorage) {
                        ...
                        this.memCache[key] = null;
                        CLDebug_1.CLDebug.Log(`D> ${key} : -----`, 'memory');
                    }
                }
            }
            catch (err) {
