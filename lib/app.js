...
function setCocktails(cocktailIds, memoryManager) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (cocktailIds.length === 0) {
            ...
            yield memoryManager.ForgetEntityAsync("suggestions");
        }
        else if (cocktailIds.length > 5) {
            yield memoryManager.ForgetEntityAsync("suggestions");
            let suggestions = yield GetSuggestions(cocktailIds, memoryManager);
            yield memoryManager.RememberEntitiesAsync("suggestions", suggestions);
            yield memoryManager.ForgetEntityAsync("noresults");
...
function Disambiguate(memoryManager, input, disambigInputs) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (refined.length === 1) {
            ...
            yield memoryManager.RememberEntityAsync("DisambigItem", input);
...
cl.AddAPICallback("Suggest", (memoryManager) => __awaiter(this, void 0, void 0, function* () {
    yield memoryManager.ForgetEntityAsync("recommend");
    ...
    if (disambigInputs.length > 0) {
        ...
        // LARS TEST await memoryManager.ForgetEntityAsync("DisambigInputs");
        //  await memoryManager.ForgetEntityAsync("DisambigItem");
        yield memoryManager.RememberEntityAsync("input", disambigInputs[choice]);
        return `I suggest ${disambigInputs[choice]}`;
    }
    // If I have things to suggest pick one
    let suggestions = yield memoryManager.EntityValueAsListAsync("suggestions");
    if (suggestions.length > 0) {
        let choice = Math.floor(Math.random() * suggestions.length);
        yield memoryManager.ForgetEntityAsync("suggestions");
        yield memoryManager.RememberEntityAsync("input", suggestions[choice]);
    ...
    if (cocktail) {
        yield memoryManager.ForgetEntityAsync("cocktails");
        yield memoryManager.RememberEntityAsync("cocktails", cocktail.idDrink);
...
cl.EntityDetectionCallback((text, memoryManager) => __awaiter(this, void 0, void 0, function* () {
    ...
    let recommend = yield memoryManager.EntityValueAsync("recommend");
    // Clear uknown
    ...
    if (!unknownInput && !recommend) {
        ...
        yield memoryManager.ForgetEntityAsync("DisambigItem");
    }
    let chosenIngredients = yield memoryManager.EntityValueAsListAsync("ingredients");
